...
public class MenuControl : CustomInput {
	...
	public void loadLevelGame(){
		if (nameLoadLevel != null) {                                                // If the name of the game level to be loaded is not empty
            //Application.LoadLevel (nameLoadLevel);									// Load game level
            SceneManager.LoadScene (nameLoadLevel, LoadSceneMode.Single);
		} else {																	// ELSE
	...
	void updateAnimScaleCredits(){
        if (animationType == CreditsAnimationType.Scale) {                              // If a scale animation to active
	...
	void updateAnimDirectionCredits(){
		if (animationType == CreditsAnimationType.Direction && _startDirAnimCredits == true) {				// If the steering animation is active and it can start
